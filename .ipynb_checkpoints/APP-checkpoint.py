...
# Fonction de nettoyage et harmonisation des catégories
...
if histo_file and budget_file:
    ...
    # Nettoyage catégories après melt
    df_resultat["Categorie"] = df_resultat["Categorie"].apply(nettoyer_categorie)

    with st.expander(f"📌 Répartion Budget_{annee_cible}"):
    ...
    ax.plot(temp_2025.index, temp_2025[f"Montant_{annee_cible}"], label={annee_cible}, marker="o")
    ax.plot(temp_moy.index, temp_moy["Montant_Moyenne"], label="Moyenne sur 3ans", linestyle="--")
    ...
    # Nettoyage catégorie AVANT fusion finale
    ...
    df_export_clean["Categorie"] = df_export_clean["Categorie"].astype(str)
    df_export_clean["Montant"] = df_export_clean["Montant"].astype(float)

    df_histo_update_clean = df_histo_update.copy()
    df_histo_update_clean["Categorie"] = df_histo_update_clean["Categorie"].astype(str)
